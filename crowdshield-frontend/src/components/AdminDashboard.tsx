...
import { AlertCircle, CheckCircle2, Shield, User, Camera, X, LogOut } from 'lucide-react';
import axios from 'axios';
import toast from 'react-hot-toast';

const AdminDashboard = () => {
  const { reports, user, updateUser } = useStore();
  const [selectedReport, setSelectedReport] = useState<any>(null);
  
  const sectorCenter = user?.city?.toLowerCase() === 'delhi' 
    ? { lat: 28.6139, lng: 77.2090 } 
    : { lat: 23.1764, lng: 80.0250 }; // Default to Jabalpur (IIITDM) center
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0A0A0A] flex flex-col overflow-hidden">
      {/* Tactical Command Strip (Top Bar) */}
      <div className="h-[60px] border-b border-white/5 flex items-center justify-center px-6 bg-[#0A0A0A]/80 backdrop-blur-xl relative z-[200]">
        ...
        <div className="flex items-center gap-6 lg:gap-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm font-black tracking-[0.2em] text-white uppercase hidden sm:block">CROWDSHIELD</h1>
          </div>

          {/* Active Alerts Badge */}
          <div className="px-4 py-1.5 bg-red-600/10 border border-red-600/20 rounded-full flex items-center gap-2.5 backdrop-blur-md">
            ...
            <span className="text-xs font-black text-white font-mono">{reports.filter(r => r.status === 'pending').length}</span>
          </div>

          {/* Operational Exit */}
          <button 
            onClick={handleLogout}
            className="flex items-center gap-2.5 px-4 h-10 hover:bg-white/5 rounded-xl text-white/40 hover:text-red-500 transition-all group"
          >
        ...
        <AnimatePresence mode="wait">
          {selectedReport && (
            <motion.div 
              ...
              className="absolute bottom-0 lg:top-10 lg:right-10 lg:bottom-10 w-full lg:w-[460px] h-[85vh] lg:h-auto glass-card rounded-t-[2.5rem] lg:rounded-[3.5rem] border border-white/10 shadow-[0_0_120px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col z-[200] max-h-[90vh] lg:max-h-none"
            >
               {/* Media Intel Banner */}
